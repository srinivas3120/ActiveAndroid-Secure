...
public abstract class Model {
  ...
  public final void delete() {
    ...
    db.delete(Cache.getTableNameWithDb(this.mTableInfo), "Id=?",
  ...
  public final void save() {
    ...
    if (this.mId == null) {
      this.mId =
          Long.valueOf(db.insert(Cache.getTableNameWithDb(this.mTableInfo), (String) null, values));
    } else {
      db.update(Cache.getTableNameWithDb(this.mTableInfo), values, "Id=" + this.mId,
          (String[]) null);
    }
    //db.close();
  ...
  public static <T extends Model> T load(Class<? extends Model> type, long id) {
    return (new Select()).from(type)
        .where("Id=?", new Object[] { Long.valueOf(id) })
        .executeSingle();
  }
                    ...
                    if (!fieldType.equals(Boolean.class) && !fieldType.equals(Boolean.TYPE)) {
                      if (!fieldType.equals(Character.class) && !fieldType.equals(Character.TYPE)) {
                        if (fieldType.equals(String.class)) {
                          ...
                          if (entity == null) {
                            entity = (new Select()).from(fieldType)
                                .where("Id=?", new Object[] { Long.valueOf(entityId) })
                                .executeSingle();
                          }
  ...
  protected final <E extends Model> List<E> getMany(Class<? extends Model> type,
      String foreignKey) {
    return (new Select()).from(type)
        .where(this.mTableInfo.getTableName() + "." + foreignKey + "=?",
  ...
  public boolean equals(Object obj) {
    ...
    return this.mId != null
        && this.mTableInfo.getTableName() == other.mTableInfo.getTableName()
        && this.mId == other.mId;
  }
}
